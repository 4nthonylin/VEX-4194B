...
int arm_cycle = 0;
int lift_angles[] = {58, 2170, 1558}; //stationary, small goal, medium goal, tall goal,
...
task arm_manager(){
	while(true){
		if(!autoarm) continue;
		//arm_change();
		if(arm_cycle < 0 || arm_cycle > 2) //ensures that the preset can never go below zero
			arm_cycle = 0;
		if(autoarm)
...
void pre_auton()
{
	...
	while(time1[T3] < 5000){
		...
		clearLCDLine(0);
		clearLCDLine(1);
		temp1 = sprintf(temp1, "Main: %1.2fv", nImmediateBatteryLevel/1000);
